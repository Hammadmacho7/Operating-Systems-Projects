...
int N;
int Bcount = 75; //count of bursts for each thread
int BcountFromFile;
char algGiven[128];
char FCFS[128] = "FCFS";
char SJF[128] = "SJF";
...
//int seedgrain = 1;
double exponentialDist(double average, double min){ //param is the random
    double uniform, num;
    do
    {
        uniform = rand() / (RAND_MAX + 1.0);
        double lambda = 1.0 / average;
        num = -log(1 - uniform) / lambda;
    } while (num < min);
    return num;
}
...
bb_rem (struct bounded_buffer *bbp)
{
    ...
    if (TRACE) {
        printf ("consumer retrieved item = %f from Thread = %d\n", qe->length, qe->threadIndex);
        fflush (stdout);
    }

    if (bbp->q->count == ( (N*Bcount) - 1))
...
producer(void * arg)
{
    ...
    for (int i = 0; i < Bcount; i++) {
       ...
       double burstTime = exponentialDist(avgA, minA);
       double burstLength = exponentialDist(avgB, minB);
...
consumer(void * arg)
{
    ...
    while( traverse != N * Bcount){
     ...
     printf(" \n%ld microseconds for consumer to terminate\n", ((end.tv_sec * 1000000 + end.tv_usec)
...
int main(int argc, char **argv) {
   ...
   if(argc == 8) {
       ...
       minA = atoi(argv[5]);
       avgA = atoi(argv[6]);
       strcpy(algGiven, argv[7]);
       if( N == 0 || N > 10){
           printf("N should be > 0 and < 11. Retry \n");
           return 0;
       }


       // N = 0; // number of W threads
       ...
       for (i = 1; i <= N; ++i) {
           ret = pthread_create(&prodtid[i], NULL, producer,
                                (void *) (long) i);
           if (ret < 0) {
       ...
       printf("Now we see what we have in the damn queue \n");
       ...
       for (int j = 1; j <= N; j++) {
           printf(" waiting time of thread %d is %f ms\n", j, waitingtime[j]);
       }
       printf("closing...\n");
       return 0;
   }
